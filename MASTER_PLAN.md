...
> 最后更新: 2026-03-08 03:12 (循环 #59 Phase 1)

## 当前阶段: ★ P5b@2000 四类全活! bus 回暖+trailer 首现; LR decay @2500 即将触发, iter 2380/6000
...
- **下次 val**: @1500 (~01:22)
- **三项修复验证**:
  - [x] 双层投影: Sequential(4096→1024→768) 生效, grad_norm 稳定 8-39
  - [x] sqrt 权重: **强力验证!** 三类同时活跃, bus 复活超 P5 全程最优
...
### ★ P5b@2000 — 四类全非零! bus 回暖, trailer 首现!

| 指标 | P5b@500 | P5b@1000 | P5b@1500 | P5b@2000 | 趋势 |
|------|---------|----------|----------|----------|------|
| car_R | 0.856 | 0.760 | 0.924 | 0.856 | ↓ car 再次让出配额 |
| car_P | 0.080 | 0.089 | 0.091 | **0.094** | ↑ P5b 全程最高 |
| truck_R | 0.153 | **0.568** | 0.390 | 0.340 | ↓ 缓降但仍达标 |
| truck_P | 0.039 | 0.061 | 0.030 | 0.035 | ↑ |
| bus_R | 0.014 | **0.368** | 0.000 | **0.085** | ↑ 从坍塌中回暖! |
| bus_P | — | 0.057 | 0.000 | **0.071** | ↑ 精度不错 |
| trailer_R | 0.000 | 0.000 | 0.000 | **0.028** | ↑ **首次非零!** |
| trailer_P | 0.000 | 0.000 | 0.000 | 0.003 | ↑ |
| bg_FA | 0.235 | 0.302 | 0.333 | **0.282** | **↓15%** 接近红线 |
| offset_cx | 0.068 | **0.049** | 0.064 | 0.055 | ↓ 接近红线 |
| offset_cy | 0.085 | 0.122 | 0.144 | **0.113** | **↓22%** 大幅改善 |
| offset_th | 0.210 | **0.168** | 0.203 | 0.208 | ~ 振荡 |

**@2000 分析**:
1. **四类全非零 — 历史首次!** car+truck+bus+trailer 同时有检出
2. **bus 回暖比 P5 快**: P5 bus@1500=0, 直到 @2500 才恢复; P5b 在 @2000 已回暖 (0.085)
3. **trailer 首现**: 72 个 GT 中检出 2 个, 虽然微弱但有信号
4. **bg_FA 持续改善**: 0.333→0.282, 距红线 0.25 仅差 0.032
5. **振荡周期 ~1000 iter**: @1000 最均衡→@1500 car主导→@2000 再次均衡化
6. **LR decay @2500 是关键转折点**: 距离 ~120 iter, 预期大幅稳定训练
...
## 红线监控
| 指标 | 红线 | @500 | @1000 | @1500 | @2000 | 趋势 | 状态 |
|------|------|------|-------|-------|-------|------|------|
| truck_R | ≥ 0.08 | 0.153✅ | 0.568✅ | 0.390✅ | 0.340✅ | ↓ 缓降但稳定达标 | ✅ |
| bg_FA | ≤ 0.25 | 0.235✅ | 0.302❌ | 0.333❌ | 0.282❌ | **↓ 改善中, 差0.032** | ❌ |
| offset_th | ≤ 0.20 | 0.210❌ | 0.168✅ | 0.203❌ | 0.208❌ | ~ 振荡 | ❌ |
| offset_cx | ≤ 0.05 | 0.068❌ | 0.049✅ | 0.064❌ | 0.055❌ | **↓ 改善中, 差0.005** | ❌ |
| offset_cy | ≤ 0.10 | 0.085✅ | 0.122❌ | 0.144❌ | 0.113❌ | **↓ 大幅改善** | ❌ |

**红线达标 1/5** (truck_R)。bg_FA 和 off_cx 均在改善, LR decay @2500 后预期显著好转。
