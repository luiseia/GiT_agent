...
> 最后更新: 2026-03-08 01:48 (循环 #58 Phase 1)

## 当前阶段: P5b@1500 类别振荡回归, bus 坍塌; BUG-19 v2 修复中, iter 1970/6000
...
### P5b@1500 — 类别振荡回归, bus 坍塌, offset 全面恶化

| 指标 | P5b@500 | P5b@1000 | P5b@1500 | P5@1500 | 趋势 |
|------|---------|----------|----------|---------|------|
| car_R | 0.856 | 0.760 | **0.924** | 0.955 | ↑ car 重新主导 |
| car_P | 0.080 | 0.089 | 0.091 | 0.057 | = |
| truck_R | 0.153 | **0.568** | 0.390 | 0.418 | **↓31%** |
| truck_P | 0.039 | 0.061 | 0.030 | 0.037 | ↓51% |
| bus_R | 0.014 | **0.368** | **0.000** | 0.000 | **↓ 坍塌!** |
| bus_P | — | 0.057 | 0.000 | 0.000 | ↓ |
| trailer_R | 0.000 | 0.000 | 0.000 | 0.000 | = |
| bg_FA | 0.235 | 0.302 | 0.333 | 0.442 | ↑ 持续恶化 |
| offset_cx | 0.068 | **0.049** | 0.064 | 0.053 | ↑ 失守红线 |
| offset_cy | 0.085 | 0.122 | 0.144 | 0.119 | ↑ 持续恶化 |
| offset_th | 0.210 | **0.168** | 0.203 | 0.201 | ↑ 失守红线 |

**@1500 分析**:
1. **类别振荡回归**: @1000 三类同活未能维持, car 重新占据预测空间, bus 坍塌至 0
2. **P5b@1500 ≈ P5@1500**: 两者指标高度相似, sqrt 权重优势在 full LR 下消退
3. **Offset 全面恶化**: cx/th 失守红线, 与类别分布剧变相关
4. **红线达标仅 1/5** (truck_R), 从 @1000 的 3/5 显著退步
5. **关键等待**: @2000 val (已触发), @2500 LR decay 是最重要验证点
...
## 红线监控
| 指标 | 红线 | P5b@500 | P5b@1000 | P5b@1500 | 趋势 | 状态 |
|------|------|---------|----------|----------|------|------|
