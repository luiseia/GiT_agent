...
> 最后更新: 2026-03-07 22:55 (循环 #52 Phase 1)

## 当前阶段: P5 训练 92% — LR decay 收敛初现! trailer_P 首超 P4!

### P5 Val 轨迹 (最近 5 个 checkpoint + P4 参照)

| 指标 | P5@3500 | P5@4000 | P5@4500 | P5@5000 | **P5@5500** | P4@4000 | 红线 |
|------|---------|---------|---------|---------|-------------|---------|------|
| car_R | 0.779 | 0.569 | 0.529 | 0.615 | **0.721** | 0.592 | — |
| car_P | 0.093 | 0.090 | 0.091 | 0.085 | **0.092** | 0.081 | — |
| truck_R | 0.679 | 0.421 | 0.317 | 0.199 | **0.203** | 0.410 | <0.08 OK |
| truck_P | 0.072 | 0.130 | 0.095 | 0.086 | **0.065** | 0.175 | — |
| bus_R | 0.120 | 0.315 | 0.058 | 0.002 | **0.014** | 0.752 | — |
| bus_P | 0.024 | 0.037 | 0.024 | 0.001 | **0.006** | 0.129 | — |
| trailer_R | 0.000 | 0.472 | 0.361 | 0.333 | **0.417** | 0.750 | — |
| trailer_P | 0.000 | 0.006 | 0.005 | 0.033 | **0.046** | 0.044 | — |
| bg_FA | 0.290 | 0.213 | 0.167 | 0.160 | **0.186** | 0.194 | ≤0.25 ✓ |
| offset_cx | 0.066 | 0.051 | 0.083 | 0.053 | **0.064** | 0.057 | ≤0.05 |
| offset_cy | 0.151 | 0.091 | 0.111 | 0.105 | **0.107** | 0.103 | ≤0.10 |
| offset_th | 0.197 | 0.142 | 0.226 | 0.163 | **0.182** | 0.207 | ≤0.20 ✓ |
...
### P5@5500 分析 — LR decay 后首次 val! 收敛信号明确!

**LR decay 效果量化 (@5000→@5500, 500 iter 收敛):**
...
| trailer_P | 0.033 | **0.046** | **↑+39% — 首超 P4!** |
| bg_FA | 0.160 | 0.186 | ↑+16% (可接受) |
| offset_th | 0.163 | 0.182 | ↑+12% (仍达标) |

**LR decay 核心效果**: 锁定已学会类别 (car, trailer), 稳定 offset 精度。代价 bg_FA 小幅回升, 可接受。

**P5 超越 P4 的指标 (@5500): 5 个!**
- car_R (+22%), car_P (+14%), trailer_P (+5%, 首超!), bg_FA (+4%), offset_th (+12%)

**未恢复**: bus_R=0.014 (仍坍塌), truck_R=0.203 (持平) — LR decay 500 iter 不足以恢复, 印证 P5b sqrt 加权必要性

**@4000 vs @5500 各有千秋**: @4000 四类全均衡+offset最优; @5500 car强+trailer_P突破+bg_FA低。@6000 最终 val (~23:17) 是关键。
...
## 红线监控
| 指标 | 红线 | P5@4000 | P5@5500 (最新) | 状态 |
|------|------|---------|---------------|------|
| truck_R | < 0.08 | 0.421 | 0.203 | 安全, 但大幅下降 |
| bg_FA | > 0.25 | 0.213 | **0.186** | **达标, 优于 P4** |
| offset_th | ≤ 0.20 | **0.142** | **0.182** | **两者均达标** |
| offset_cy | ≤ 0.10 | **0.091** | 0.107 | @4000 达标 |
| offset_cx | ≤ 0.05 | **0.051** | 0.064 | @4000 接近 |
| avg_P | ≥ 0.20 | 0.066 | 0.052 | 差距大, P5b 解决 |
