#!/usr/bin/env python
"""
Two-Stage v2: Stage 2 uses OR logic to protect small objects.

Stage 1 (object-level): visibility >= min_vis
Stage 2 (cell-level):   KEEP if IoF >= min_iof  OR  IoB_cell >= min_iob
  - IoF  = intersection / cell_area   (大物体:cell被bbox覆盖了多少?)
  - IoB  = intersection / bbox_area   (小物体:bbox有多大比例落在这个cell?)

小目标bbox < cell → IoF 天然低, 但 IoB 高 → 被 OR 的第二条件保护
大目标边缘cell → IoF 低, IoB 也低 → 被过滤
"""
...
SAVE_DIR = osp.join(GIT_ROOT, 'ssd_workspace/VIS/two_stage_v2/')
...
CONFIGS = [
    {'name': 'baseline',           'min_vis': 0.0,  'min_iof': 0.0, 'min_iob': 0.0},
    {'name': 'v10+IoF20',          'min_vis': 0.10, 'min_iof': 0.20, 'min_iob': 0.0},
    {'name': 'v10+IoF20|IoB20',    'min_vis': 0.10, 'min_iof': 0.20, 'min_iob': 0.20},
    {'name': 'v10+IoF20|IoB30',    'min_vis': 0.10, 'min_iof': 0.20, 'min_iob': 0.30},
    {'name': 'v15+IoF20|IoB20',    'min_vis': 0.15, 'min_iof': 0.20, 'min_iob': 0.20},
    {'name': 'v10+IoF20|IoB50',    'min_vis': 0.10, 'min_iof': 0.20, 'min_iob': 0.50},
]
...
def compute_object_cells(min_u, max_u, min_v, max_v, cur_w, cur_h):
    ...
    if mu_c >= xu_c or mv_c >= xv_c:
        ...
        if 0<=cc<GRID_FINE_W and 0<=cr<GRID_FINE_H:
            return [{'id':cr*GRID_FINE_W+cc,'iof':1.0,'iob':1.0}], vis, clamp_area
        return [], vis, clamp_area
    ...
    for r in range(rs,re+1):
        for c in range(cs,ce+1):
            ...
            iof = inter/cell_area
            iob = inter/clamp_area  # use clamped bbox area (vis filter handles out-of-frame)
            results.append({'id':r*GRID_FINE_W+c, 'iof':iof, 'iob':iob})
    return results, vis, clamp_area
...
def project_objects(info):
    ...
    for g in range(len(gt_boxes)):
        ...
        cells, vis, clamp_area = compute_object_cells(mu,xu,mv,xv,cw,ch)
        ...
        objects.append({
            ...
            'bbox_vs_cell': clamp_area/CELL_AREA,  # bbox size relative to cell
            'n_total': len(cells),
        })
...
def apply_filter(objects, min_vis, min_iof, min_iob):
    ...
    for obj in objects:
        if obj['vis'] < min_vis:
            n_vis_rej += 1; cells_per_obj.append(0); continue
        # OR logic: keep if IoF >= min_iof OR IoB >= min_iob
...
def draw_filtered(vis_base, objects, result, config_name, cfg, cur_w, cur_h):
    vis = vis_base.copy()
    fw=float(cur_w)/GRID_FINE_W; fh=float(cur_h)/GRID_FINE_H
    ...
    r=result
    cv2.putText(vis,config_name,(10,28),cv2.FONT_HERSHEY_SIMPLEX,0.50,(0,255,255),2)
...
def process_sample(info, token, save_dir):
    ...
    for cfg in CONFIGS:
        result = apply_filter(objects, cfg['min_vis'], cfg['min_iof'], cfg['min_iob'])
        all_results.append(result)
        v = draw_filtered(vis_img, objects, result, cfg['name'], cfg, cw, ch)
        images.append(v)
        safe = cfg['name'].replace(' ','_').replace('|','or').replace('>=','ge')
        cv2.imwrite(osp.join(save_dir,f'{token}_{safe}.jpg'),v,[cv2.IMWRITE_JPEG_QUALITY,95])

    scaled = [cv2.resize(im,(560,560)) for im in images]
    row1=np.hstack(scaled[:3]); row2=np.hstack(scaled[3:])
    combined=np.vstack([row1,row2])
    cv2.imwrite(osp.join(save_dir,f'{token}_compare.jpg'),combined,[cv2.IMWRITE_JPEG_QUALITY,95])

    # Print detail
    print(f"\n{'='*110}")
    print(f"Sample: {token}")
    print(f"{'='*110}")
    hdr = f"{'Class':>14} {'Bbox':>10} {'bv/c':>5} {'Vis%':>5} {'#C':>3}"
    for cfg in CONFIGS: hdr += f" {cfg['name'][:14]:>14}"
    print(hdr); print('-'*110)

    for i,obj in enumerate(objects):
        fw_,fh_=obj['full_size']; bvc=obj['bbox_vs_cell']
        line = f"{obj['cls']:>14} {int(fw_):>4}x{int(fh_):<4} {bvc:>5.1f} {obj['vis']*100:>4.0f}% {obj['n_total']:>3}"
        for j,cfg in enumerate(CONFIGS):
            cpo = all_results[j]['cells_per_obj'][i]
            # Mark with * if saved by IoB condition
            marker = ''
            if cfg['min_iob'] > 0 and cpo > 0:
                # Check if any cell was saved purely by IoB (not IoF)
                if obj['vis'] >= cfg['min_vis']:
                    iob_saves = sum(1 for c in obj['cells']
                                    if c['iof'] < cfg['min_iof'] and c['iob'] >= cfg['min_iob'])
                    if iob_saves > 0: marker = f'(+{iob_saves})'
            line += f" {cpo:>7}{marker:>7}"
        print(line)

    print(f"\n{'TOTAL FG':>14} {'':>10} {'':>5} {'':>5} {'':>3}", end='')
    for r in all_results: print(f" {r['total_fg']:>14}", end='')
    print()
    return all_results
...
def main():
    ...
    for token,info in samples:
        ...
        for i,r in enumerate(results):
            agg[i]['fg']+=r['total_fg']; agg[i]['kept']+=r['n_kept']
            agg[i]['total']+=r['n_obj']; agg[i]['vis_rej']+=r['n_vis_rej']
            agg[i]['iof_lost']+=r['n_iof_lost']
    ...
    print(f"{'='*90}")
    print(f"{'Config':>24} {'AvgFG':>7} {'Kept%':>7} {'VisRej%':>8} {'IoFLost%':>9} {'AvgC/obj':>9}")
    print('-'*70)
    for i,cfg in enumerate(CONFIGS):
        a=agg[i]; af=a['fg']/max(n,1)
        kp=100*a['kept']/max(a['total'],1)
        vr=100*a['vis_rej']/max(a['total'],1)
        il=100*a['iof_lost']/max(a['total']-a['vis_rej'],1)
        ac=np.mean([c for c in a['cpo'] if c>0]) if any(c>0 for c in a['cpo']) else 0
        print(f"{cfg['name']:>24} {af:>7.1f} {kp:>6.1f}% {vr:>7.1f}% {il:>8.1f}% {ac:>9.1f}")
