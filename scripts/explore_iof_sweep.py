#!/usr/bin/env python
"""
IoF Sweep: Fix vis>=10%, IoB>=20%, sweep IoF from 10% to 40%.
Visualize the effect of different IoF thresholds on filtering.
"""
...
SAVE_DIR = osp.join(GIT_ROOT, 'ssd_workspace/VIS/iof_sweep/')
...
# Fixed: vis>=10%, IoB>=20%.  Sweep IoF.
CONFIGS = [
    {'name': 'baseline',           'min_vis': 0.0,  'min_iof': 0.0,  'min_iob': 0.0},
    {'name': 'v10+IoF10|IoB20',    'min_vis': 0.10, 'min_iof': 0.10, 'min_iob': 0.20},
    {'name': 'v10+IoF15|IoB20',    'min_vis': 0.10, 'min_iof': 0.15, 'min_iob': 0.20},
    {'name': 'v10+IoF20|IoB20',    'min_vis': 0.10, 'min_iof': 0.20, 'min_iob': 0.20},
    {'name': 'v10+IoF25|IoB20',    'min_vis': 0.10, 'min_iof': 0.25, 'min_iob': 0.20},
    {'name': 'v10+IoF30|IoB20',    'min_vis': 0.10, 'min_iof': 0.30, 'min_iob': 0.20},
    {'name': 'v10+IoF35|IoB20',    'min_vis': 0.10, 'min_iof': 0.35, 'min_iob': 0.20},
    {'name': 'v10+IoF40|IoB20',    'min_vis': 0.10, 'min_iof': 0.40, 'min_iob': 0.20},
]
...
def project_objects(info):
    ...
    for g in range(len(gt_boxes)):
        ...
        objects.append({
            ...
            'bbox_vs_cell': clamp_area/CELL_AREA,
            'n_total': len(cells),
        })
...
def process_sample(info, token, save_dir):
    ...
    row1=np.hstack(scaled[:4]); row2=np.hstack(scaled[4:])
    ...
    for cfg in CONFIGS: hdr += f" {cfg['name'][:16]:>16}"
    print(hdr); print('-'*140)
    ...
    for i,obj in enumerate(objects):
        ...
        for j,cfg in enumerate(CONFIGS):
            ...
            line += f" {cpo:>9}{marker:>7}"
        print(line)

    print(f"\n{'TOTAL FG':>14} {'':>10} {'':>5} {'':>5} {'':>3}", end='')
    for r in all_results: print(f" {r['total_fg']:>16}", end='')
    print()
    return all_results
